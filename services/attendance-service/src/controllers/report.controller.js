...
async function rekapBulanan(req, res) {
  ...
  const ringkasan = rows.reduce(
    (acc, r) => ({
      ...
      total_hadir:      acc.total_hadir  + Number(r.total_hadir  || 0),
      total_telat:      acc.total_telat  + Number(r.total_telat  || 0),
      total_alpha:      acc.total_alpha  + Number(r.total_alpha  || 0),
      total_cuti:       acc.total_cuti   + Number(r.total_cuti   || 0),
      total_poin:       acc.total_poin   + Number(r.total_poin   || 0),
    }),
  ...
  return res.status(200).json({
    ...
    data: {
      ...
      pegawai:   rows.map((r) => ({
        ...
        hadir:                Number(r.total_hadir)      || 0,
        telat:                Number(r.total_telat)      || 0,
        alpha:                Number(r.total_alpha)      || 0,
        cuti:                 Number(r.total_cuti)       || 0,
        libur:                Number(r.total_libur)      || 0,
        poin_kehadiran:       Number(r.total_poin)       || 0,
        rata_terlambat_menit: Number(r.rata_terlambat_menit) || 0,
      })),
    },
  });
}
